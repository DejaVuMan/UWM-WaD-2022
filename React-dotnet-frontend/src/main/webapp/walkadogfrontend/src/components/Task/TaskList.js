...
class TaskList extends Component {
    ...
    findAllTasks(obecnaStrona) {
        obecnaStrona -= 1;
        axios.get("http://localhost:4000/tasks?pageNumber="+obecnaStrona+"&pageSize="+this.state.taskiNaStrone+"&sortBy=tytul&sortDir="+this.state.sortDir)
    ...
    wyszukajDane = (obecnaStrona) => {
        obecnaStrona -= 1;
        axios.get("http://localhost:4000/tasks/wyszukaj/"+this.state.wyszukaj+"?page="+obecnaStrona+"&size="+this.state.taskiNaStrone)
            .then(response => response.data)
            .then((data)=>{
                this.setState({
                    tasks: data.content,
        ...
        return(
            <div>
                ...
                <div style={{"display": this.state.show ? "block" : "none"}}>
                    <MyToast show={this.state.show} message={"Pies został usunięty pomyślnie"} type={"danger"}/>
                </div>
            </div>
        )
    }
